...
class TransRec(Model):
    ...
    def forward(self, cur_user, prev_item, pos_item, neg_item):
        ...
        reg = tf.contrib.layers.l2_regularizer(scale=opts.reg)
        ...
        reg_user = tf.contrib.layers.l2_regularizer(scale=opts.user_reg)
        user_emb = tf.get_variable(
            initializer=tf.zeros([corp.n_users, opts.emb_dim]), regularizer=reg_user, name="user_emb")
        self._user_emb = user_emb

        # Item bias: [n_items]
        reg_bias = tf.contrib.layers.l2_regularizer(scale=opts.bias_reg)
        item_bias = tf.get_variable(
            initializer=tf.zeros([corp.n_items]), regularizer=reg_bias, name="item_bias")
    ...
    def save_params_graph(self):
        ...
        self._item_emb_best = tf.get_variable(
            shape=[corp.n_items, opts.emb_dim],
            trainable=False,
            name="item_emb_best")
    ...
    def one_iter(self):
        ...
        for b_no in range(batch_num):
            ...
            # Batch Begin!
            for ind in range(total_num)[b_no * batch_size:(b_no + 1) * batch_size]:
                if ind % 1000 == 0:
                    print('.', end='')
                    sys.stdout.flush()
                # Sample a user
    ...
    def show_params(self, u, prev_i, pos_i, neg_i):
        corp = self._corpus
        print()
        print("  @all_user_emb\n  ", end='')
