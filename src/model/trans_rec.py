...
class TransRec(Model):
    ...
    def forward(self, cur_user, prev_item, pos_item, neg_item):
        ...
        self._norm_all_user_emb_op = tf.assign(all_user_emb, tf.nn.l2_normalize(all_user_emb, dim=0, epsilon=1.0))
        ...
        self._norm_item_emb_op = tf.assign(item_emb, tf.nn.l2_normalize(item_emb, dim=1, epsilon=1.0))
    ...
    def train_step(self, users, prev_items, pos_items, neg_items):
        opts = self._options
        feed_dict = {
    ...
    def get_train_batch(self, batch_no):
        opts = self._options
        corp = self._corpus
        batch_size = opts.train_batch_size
        users = np.random.choice(self._train_legal_user, replace=True, size=batch_size)
        prev_ind = (np.random.rand(batch_size) * (self._len_per_user[users] - 1)).astype(int)
        pos_ind = prev_ind + 1
        neg_items = np.random.randint(0, corp.n_items, size=batch_size)
        prev_items, pos_items = list(), list()
        processed_num = batch_size * batch_no
        for i, u in enumerate(users):
            if i + processed_num % 10000 == 0:
                print('.', end='')
                sys.stdout.flush()
            prev_items.append(corp.pos_per_user[u][prev_ind[i]]['item'])
            pos_items.append(corp.pos_per_user[u][pos_ind[i]]['item'])
            while neg_items[i] in self._clicked_per_user[u]:
                neg_items[i] = np.random.randint(0, corp.n_items)
    ...
    def one_iter(self):
        ...
        batch_size, total_num = opts.train_batch_size, corp.n_clicks
        ...
        for b_no in range(batch_num):
            ...
            users, prev_items, pos_items, neg_items = self.get_train_batch(b_no)
            prepare_time += time.time() - prepare_start

            session_start = time.time()
            l1, l2 = self.train_step(users, prev_items, pos_items, neg_items)
        ...
        self._session.run([self._norm_item_emb_op])
    ...
    def train(self):
        opts = self._options
        self._session.run([self._norm_all_user_emb_op, self._norm_item_emb_op])
